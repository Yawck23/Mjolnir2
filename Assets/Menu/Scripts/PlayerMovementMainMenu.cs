...
public class PlayerMovementMainMenu : MonoBehaviour
{
    ...
    [SerializeField] private float waitAfterJump = 3f;

    private IEnumerator Move(float duration)
    {
        _animator.SetTrigger("MoveStart");
        float elapsedTime = 0f;
        moveTimer = movementDuration;
        while (elapsedTime < duration)
        {
            elapsedTime += Time.deltaTime;
            _direction = new Vector3(transform.forward.x, 0.0f, transform.forward.z);
    ...
    private void CalcularAceleracion()
    {
        ...
        if (moveTimer > 0.01f)  // Si hay input, acelerar
        {
